...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      </div>      {/* Scroll Indicator - Only for card view */}
      {viewMode === 'card' && (
      ...
      {/* Content Container */}
      {viewMode === 'card' ? (
        /* Card View - Full screen scrolling */
        <div 
          ref={containerRef}
        ...
        <div className={`min-h-screen transition-all duration-300 ${
          ...
          {filteredNotes.length === 0 ? (
            ...
            <div className="max-w-2xl mx-auto px-4 space-y-4">
              {filteredNotes.map((note) => (
                <div key={note.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 hover:shadow-md transition-shadow cursor-pointer" onClick={() => handleCardTap(note.id)}>
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <h3 className="text-lg font-semibold text-gray-900 mb-2">{note.title}</h3>
                      <p className="text-gray-600 text-sm line-clamp-2">{note.content.substring(0, 150)}...</p>
                    ...
                    <button
                      ...
                      className="ml-4 flex-shrink-0 w-8 h-8 bg-blue-600 text-white rounded-full shadow-sm hover:bg-blue-700 transition-colors flex items-center justify-center"
                    >
          ...
          {filteredNotes.length === 0 ? (
            ...
            <div className="max-w-6xl mx-auto px-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredNotes.map((note) => (
                  <div key={note.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 hover:shadow-md transition-all hover:scale-105 cursor-pointer" onClick={() => handleCardTap(note.id)}>
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <button
                        ...
                        className="ml-2 flex-shrink-0 w-8 h-8 bg-blue-600 text-white rounded-full shadow-sm hover:bg-blue-700 transition-colors flex items-center justify-center"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                    ...
                    <p className="text-gray-600 text-sm line-clamp-3 mb-3">{note.content.substring(0, 100)}...</p>
